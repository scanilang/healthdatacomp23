...
model_data <- combined_w_county %>%
  select(Actual.Rate, police_score, sk2014, covid_rate, educ_pct, mean_walk_score, homeownershiprate, code) %>%
  mutate(code = as.factor(code))
```

```{r}
# Too much data missing. Gotta do imputation or something
mod1 <- lm(data = model_data, Actual.Rate ~ .)
summary(mod1)
```
...
imputed <- mice(model_data, method = "norm", m = 50, maxit = 10)
